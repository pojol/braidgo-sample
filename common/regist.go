...
func Regist(req ConsulRegistReq, consulAddr string) (err error) {
	byt, _ := json.Marshal(&req)
	reqBuf := bytes.NewBuffer(byt)
	client := &http.Client{}

	httpReq, err := http.NewRequest("PUT", consulAddr+"/v1/agent/service/register", reqBuf)
	if err != nil {
		return err
	}
...
func Deregist(id string, consulAddr string) error {
	client := &http.Client{}
	httpReq, err := http.NewRequest("PUT", consulAddr+"/v1/agent/service/deregister/"+id, nil)
	if err != nil {
		return err
	}
	httpReq.Header.Set("Content-type", "application/json")
